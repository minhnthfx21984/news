...
buttonSubmit.addEventListener("click", function () {
    if (inputQuery.value == "") {
        alert(
            `Search field can't blank\n
            I just show all posts with default settings`
        );
        if (requestParameters.q) {
            delete requestParameters.q;
            renderAllPost(requestParameters);
        }
        return;
    }
    requestParameters.q = inputQuery.value;
    renderAllPost(requestParameters);
});

// BUTTON NEXT PAGE ON CLICK
buttonNext.addEventListener("click", function () {
    if (requestParameters.page < totalPost / requestParameters.pageSize) {
...
function renderAllPost(requestParameters) {
    getData(requestParameters)
        .then((data) => {
            ...
            checkPage(); //check page and hide button
